...
export const LESSON = {
  /**
   * Доля видео, после которой урок можно завершить и он считается «просмотренным»:
   * открывает следующий урок (в курсе со строгим порядком) и — с учётом ДЗ — конспекты.
   * Требуется полный просмотр (100%).
   */
  WATCHED_THRESHOLD: 1.0,
} as const;
...
export const SIGNED_URL = {
  /**
   * TTL ссылки на видео-поток, секунд. Должен перекрывать всю длину урока + паузы/перемотки
   * в рамках одного сеанса: HLS догружает сегменты тем же токеном по ходу просмотра, и если
   * токен протухнет посреди ролика — Bunny начнёт отдавать 403 на сегменты (видео «подвисает»).
   * Ссылка пересоздаётся при каждом открытии урока и гейтится доступом, поэтому 4 ч безопасны.
   */
  VIDEO_TTL_SEC: 60 * 60 * 4,
...
export const UPLOAD = {
  ...
  /**
   * Потолок размера конспекта при загрузке через сервер. Жёстко ограничен платформой
   * Vercel (тело запроса ~4.5 МБ) — берём 4 МБ с запасом. Для бóльших файлов нужен
   * прямой аплоад в хранилище (отдельная задача).
   */
  MAX_LESSON_FILE_BYTES: 4 * 1024 * 1024,
} as const;
